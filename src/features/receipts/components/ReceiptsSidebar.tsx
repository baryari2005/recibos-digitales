...
type ReceiptTab = "pendientes" | "firmados";

function isReceiptTab(value: string): value is ReceiptTab {
  return value === "pendientes" || value === "firmados";
}
        ...
        <CardContent className="p-0">
          ...
          {!loading && docs.length > 0 && docs.map(d => (
            <button key={d.id}
              ...
              className={`w-full text-left p-4 border-l-4 transition-colors ${selectedId === d.id ? "border-teal-500 bg-muted/40" : "border-transparent hover:bg-muted/30"
                }`}>
